...
public class NIOSSLBasicTest {
    ...
    public void basicConnector() throws Exception {
        BrokerService broker = createBroker("nio+ssl", "nio+ssl://localhost:0?transport.needClientAuth=true&transport.verifyHostName=false");
    ...
    public void enabledCipherSuites() throws Exception {
        BrokerService broker = createBroker("nio+ssl", "nio+ssl://localhost:0?transport.needClientAuth=true&transport.verifyHostName=false&transport.enabledCipherSuites=SSL_RSA_WITH_RC4_128_SHA,SSL_DH_anon_WITH_3DES_EDE_CBC_SHA");
        basicSendReceive("ssl://localhost:" + broker.getConnectorByName("nio+ssl").getConnectUri().getPort() + "?socket.verifyHostName=false");
        stopBroker(broker);
    }
    ...
    public void verifyHostNameError() throws Exception {
        ...
        try {
            broker = createBroker("nio+ssl", "nio+ssl://localhost:61616?transport.needClientAuth=true");
